...
const require$$0$1 = require('assert');
...
const vscode__default = /*#__PURE__*/_interopDefault(vscode);
const require$$0__default = /*#__PURE__*/_interopDefault(require$$0);
const os__default = /*#__PURE__*/_interopDefault(os);
const path__default = /*#__PURE__*/_interopDefault(path$3);
...
function vscodeAbortController(token) {
...
function getConfig(scope) {
  return vscode__default.default.workspace.getConfiguration("fish-completion", scope);
}
function getFishPath(scope) {
  return getConfig(scope).get("path.fish") ?? "";
}
function isAssistantEnabled(scope) {
  return getConfig(scope).get("assistant.v1.enabled") ?? false;
}

const disposables = /* @__PURE__ */ new Set();

const output = vscode__default.default.window.createOutputChannel("Fish Completion");
...
const temporaryDir = require$$0.mkdtempSync(path$3.join(os.tmpdir(), "fish-completion-"));
disposables.add(
  new vscode__default.default.Disposable(() => {
    require$$0.rmSync(temporaryDir, { force: true, recursive: true });
  })
);
const safePath = /^(?:\/[\w.-]+)+$/;
function startWorker(options) {
  ...
  const child = execa(
    ...
    ],
    {
      ...
      env: {
        ...
        TERM: "dumb",
        GIO_LAUNCHED_DESKTOP_FILE: void 0,
        VISUAL: void 0,
        CHROME_DESKTOP: void 0,
        VSCODE_GIT_ASKPASS_NODE: void 0,
        VSCODE_GIT_ASKPASS_MAIN: void 0,
        TERM_PROGRAM: void 0,
        GIT_ASKPASS: void 0,
        VSCODE_GIT_IPC_HANDLE: void 0,
        EDITOR: void 0,
        VSCODE_GIT_ASKPASS_EXTRA_ARGS: void 0,
        _FISH_COMPLETION_ASSIST: options.isAssistantEnabled ? "v1" : "disabled",
        _FISH_COMPLETION_TEMP_DIR: temporaryDir,
        _FISH_COMPLETION_WORKER: url.fileURLToPath(
          new URL("worker.fish", (typeof document === 'undefined' ? new (require('u' + 'rl').URL)('file:' + __filename).href : (document.currentScript && document.currentScript.src || new URL('extension.js', document.baseURI).href)))
        )
  ...
  return {
    ...
    outputChannel: child.stdio[9]
  };
}
function debugStdOutputAndError(child, output) {
  for (const channel of ["stdout", "stderr"]) {
    const rl = readline.createInterface(child[channel]);
    rl.on("line", (line) => {
...
async function updateCompletions(options) {
  const state = {
    progress: 0,
    total: 1,
    current: "Updating\u2026"
  ...
  const rl = readline.createInterface(outputChannel);
  ...
  rl.on("line", (line) => {
    ...
    const match = /^\s*(?<progress>\d+)\s*\/\s*(?<total>\d+)\s*:\s*(?<current>.+?)\s*$/.exec(
      line
    );
    if (!match) {
      return;
...
const command = async (progress, token) => {
  if (token.isCancellationRequested) {
    return;
  }
  ...
  try {
    ...
    void vscode__default.default.window.showErrorMessage(
...
function registerUpdateCompletionsCommand() {
  disposables.add(
    vscode__default.default.commands.registerCommand(
...
const variableExcludeList = /* @__PURE__ */ new Set([
  "argv",
  "CMD_DURATION",
  "COLUMNS",
  "fish_bind_mode",
...
const functionExcludeList = /* @__PURE__ */ new Set([
  "fish",
  "fish_indent",
...
function getEndOfLine(eol) {
  switch (eol) {
    case vscode__default.default.EndOfLine.LF: {
      return "\n";
    }
    case vscode__default.default.EndOfLine.CRLF: {
      return "\r\n";
    }
    default: {
...
const functionPattern = /^\s*function\s+(?:-[^\s&|<>;()]*\s+)*(?<name>[\w[\].+-]+)?/;
...
function getAssistantResult(document) {
  fixCache();
...
async function completeCommand(options) {
  await promises.writeFile(path$3.join(temporaryDir, "text"), options.text, "utf8");
  await promises.writeFile(
    path$3.join(temporaryDir, "cmd"),
  ...
  try {
    ...
    await promises.writeFile(path$3.join(temporaryDir, "text"), "", "utf8");
    await promises.writeFile(path$3.join(temporaryDir, "cmd"), "", "utf8");
  }
  output.appendLine("Done");
  return { completions, currentToken };
}

const startOfDocument = new vscode__default.default.Position(0, 0);
...
const completionProvider = {
  async provideCompletionItems(document, position, token) {
    ...
    const { signal, dispose } = vscodeAbortController(token);
    try {
      const assistantCommands = isAssistantEnabled(document) ? getAssistantResult(document).getCommands() : "";
      const text = document.getText(
        new vscode__default.default.Range(startOfDocument, position)
      );
      const { completions, currentToken } = await completeCommand({
        cwd: path$3.dirname(document.uri.fsPath),
...
function getCompletionItem(options) {
  const completion = new vscode__default.default.CompletionItem(
    {
      label: options.label,
  ...
  completion.range = new vscode__default.default.Range(
...
function getCompletionKind(type) {
  switch (type) {
    case "File": {
      return vscode__default.default.CompletionItemKind.File;
    }
    case "Folder": {
      return vscode__default.default.CompletionItemKind.Folder;
    }
    case "Keyword": {
      return vscode__default.default.CompletionItemKind.Keyword;
    }
    case "Function": {
      return vscode__default.default.CompletionItemKind.Function;
    }
    case "Variable": {
      return vscode__default.default.CompletionItemKind.Variable;
    }
    default: {
      return vscode__default.default.CompletionItemKind.Text;
    }
  }
}
function registerCompletionProvider() {
  disposables.add(
    vscode__default.default.languages.registerCompletionItemProvider(
      "fish",
      completionProvider
    )
  );
